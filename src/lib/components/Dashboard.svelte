<script lang="ts">
	...
	let scanning = false;
	// Flag to track if the protest note has already been displayed automatically
	let protestNoteShown = false;
	...
	function openProtestNoteModal() {
     ...
     <h2>Protest Note – Against Trademark Trolling</h2>
<p>Dear Legal Departments and Corporate‑Control Bots,</p>
<p>
  We, the developers of this world, hereby solemnly declare:
</p>
<ol>
  <li><strong>“Dev Mode” belongs to everyone!</strong> Since time immemorial, we developers have been activating …
  </li>
  <li>A trademark application for “Dev Mode” is like trying to protect the word <em>“breath”</em> …
  </li>
  <li>We demand: Immediate release of all ™ symbols! End the patented pixel glitch!
  </li>
</ol>
<p><em>Action:</em> From now on, every user of this project can mark all terms in “™ Safe Mode” …</p>
<p>In programmatic protest,<br>Your <strong>Dev Mode</strong> Team™</p>
	...
	$: if ($safeMode && $trademarkRadarActive && !$complianceMode && !protestNoteShown) {
		...
		protestNoteShown = true; // Mark that we've shown the protest note
		setTimeout(() => {
			if (!$modalStore.open) {
...
<div class="dashboard">
	...
	<footer class="panel dashboard-footer">
		...
		<p class="footer-links">
			<button class="protest-link" on:click={openProtestNoteModal}>Protest Note</button>
		</p>
	</footer>
</div>
